...
const heroStats: { label: string; value: string; subtitle?: string }[] = [
  { label: "Security Posture Improved", value: "89%", subtitle: "avg. clients" },
  { label: "Recovery Time", value: "4h", subtitle: "faster than avg." },
  { label: "Years Combined Expertise", value: "150+" },
];
...
export function GlowyWavesHero() {
  ...
  return (
    <section
      ...
      role="region"
      aria-label="Hero section with animated waves"
    >
      ...
      <div className="absolute inset-0 -z-10 pointer-events-none">
        <div className="absolute left-1/2 top-0 h-[520px] w-[520px] -translate-x-1/2 rounded-full bg-primary/[0.08] blur-[140px] dark:bg-primary/[0.12]" />
        <div className="absolute bottom-0 right-0 h-[360px] w-[360px] rounded-full bg-accent/[0.05] blur-[120px] dark:bg-accent/[0.08]" />
        <div className="absolute top-1/2 left-1/4 h-[400px] w-[400px] rounded-full bg-secondary/[0.04] blur-[150px] dark:bg-secondary/[0.06]" />
      </div>
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-6xl flex-col items-center px-6 py-24 text-center md:px-8 lg:px-12">
        ...
        >
          ...
          >
            Enterprise-grade cybersecurity and GRC consulting. We help you manage risk, achieve compliance, and defend against modern threats with strategic clarity.
          </motion.p>
          ...
          <motion.ul
            variants={itemVariants}
            className="mb-16 flex flex-wrap items-center justify-center gap-3 text-xs uppercase tracking-[0.2em] text-foreground/70 dark:text-foreground/80"
          >
            {[
              { icon: Shield, text: "Zero-Trust Architecture" },
              { icon: Zap, text: "24/7 Threat Detection" },
              { icon: Lock, text: "Compliance Automation" },
            ].map(({ icon: Icon, text }) => (
              <li
                key={text}
                className="flex items-center gap-2 rounded-full border border-border/40 bg-background/60 px-4 py-2 backdrop-blur dark:border-border/60 dark:bg-background/70"
              >
